const TRANSLATIONS = {
  ...
  welcomeMessage: {
    part1:
      "Добро пожаловать в , открытый инструмент искусственного интеллекта от Athena, который превращает что угодно в обученный чат-бот, с которым вы можете общаться и задавать вопросы.  - это ПО BYOK (принеси свои собственные ключи), поэтому за использование этого ПО нет подписки, платы или других сборов, кроме тех, что вы хотите использовать.",
    part2:
      " - это самый простой способ объединить мощные продукты ИИ, такие как OpenAi, GPT-4, LangChain, PineconeDB, ChromaDB и другие сервисы, в аккуратный пакет без лишних хлопот, чтобы повысить вашу продуктивность в 100 раз.",
    part3:
      " может работать полностью локально на вашем компьютере с минимальной нагрузкой, вы даже не заметите его присутствия! GPU не требуется. Также доступна установка в облаке и на локальных серверах.\nЭкосистема инструментов ИИ становится мощнее с каждым днем.  упрощает их использование.",
  ...
  api: {
    title: "API ключи",
    description:
      "API ключи позволяют владельцу программно получать доступ к этому экземпляру  и управлять им.",
  ...
  llm: {
    ...
    description:
      "Это учетные данные и настройки для вашего предпочтительного поставщика чата и встраивания LLM. Важно, чтобы эти ключи были актуальными и правильными, иначе  не будет работать должным образом.",
  ...
  transcription: {
    ...
    "warn-start":
      "Использование локальной модели whisper на машинах с ограниченной оперативной памятью или процессором может привести к зависанию  при обработке медиафайлов.",
    "warn-recommend":
  ...
  embedding: {
    ...
    "desc-end":
      "Встраивание - это процесс превращения текста в векторы. Эти учетные данные необходимы для превращения ваших файлов и подсказок в формат, который  может использовать для обработки.",
    provider: {
      title: "Поставщик встраивания",
      description:
        "Нет необходимости в настройке при использовании встроенного механизма встраивания .",
    },
  },
  text: {
  ...
  vector: {
    ...
    description:
      "Это учетные данные и настройки для того, как будет функционировать ваш экземпляр . Важно, чтобы эти ключи были актуальными и правильными.",
    provider: {
  ...
  multi: {
    ...
    password: {
      ...
      description:
        "Защитите ваш экземпляр  паролем. Если вы забудете его, метода восстановления не существует, поэтому убедитесь, что вы сохранили этот пароль.",
    },
    instance: {
  ...
  privacy: {
    ...
    description:
      "Это ваша конфигурация для того, как подключенные сторонние поставщики и  обрабатывают ваши данные.",
    llm: "Выбор LLM",
